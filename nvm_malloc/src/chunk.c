...
#ifdef PMEM_ENABLED
#define MMAP_FLAGS MAP_SYNC|MAP_SHARED_VALIDATE|MAP_FIXED
#else
#define MMAP_FLAGS MAP_SHARED|MAP_FIXED
#endif
...
int nvm_fallocate(int fd, off_t offset, off_t len) {
#ifdef __linux
...
uint64_t recover_chunks() {
    ...
    if (mmap(next_chunk_addr, n_bytes, PROT_READ|PROT_WRITE, MMAP_FLAGS, backing_file_fd, 0) == MAP_FAILED) {
...
void* activate_more_chunks(uint64_t n_chunks) {
#ifdef LOGGING
    printf("Activating more chunks%lu\n", n_chunks);
#endif
    ...
    if (nvm_fallocate(backing_file_fd, next_chunk*CHUNK_SIZE, n_chunks*CHUNK_SIZE) != 0) {
        printf ("Error fallocate");
        error_and_exit("unable to increase file size of %s", backing_file_path);
    }
    if (mmap(next_chunk_addr, n_chunks*CHUNK_SIZE, PROT_READ|PROT_WRITE, MMAP_FLAGS, backing_file_fd, next_chunk*CHUNK_SIZE) == MAP_FAILED) {
        printf ("Error mapping chunks\n");
        error_and_exit("error mapping chunks");
    }

    /* Pre-faultin all pages */
    volatile char *addr = (char*) next_chunk_addr;
    long ctr = 0;
    for (long i = 0; i < n_chunks*CHUNK_SIZE; i += 4096) {
