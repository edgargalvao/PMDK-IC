...
void *nvm_start = NULL;
uint64_t size_requested = 0;
uint64_t size_allocated = 0;
uint64_t size_deallocated = 0;
uint64_t num_allocated = 0;
...
void* nvm_reserve(uint64_t n_bytes) {
    ...
    size_requested += n_bytes;
    size_allocated += (n_bytes & ~63) + (n_bytes % 64 != 0 ? 64 : 0);
    num_allocated +=1;

#ifdef LOGGING
    if (num_allocated % (100000ul)==0) {
        nvm_report_mem();
    }
#endif
...
void nvm_activate(void *ptr, void **link_ptr1, void *target1, void **link_ptr2, void *target2) {
    ...
    if (rel_ptr % CHUNK_SIZE == sizeof(nvm_huge_header_t)) {
        ...
        if (GET_USAGE(nvm_block->state) == USAGE_FREE) {
            ...
            // These sfences are needed only if there are linkptr imo.
            //sfence();
...
void nvm_free_size(void *ptr, uint64_t n_bytes) {
    size_deallocated += n_bytes; 
    nvm_free(ptr, NULL, NULL, NULL, NULL);
}
...
void nvm_report_mem() {
    printf("Num allocated: %lu\n", num_allocated);
    printf("Requested/Allocated: %f/%f KB\n",
            (size_requested/1024.0), (size_allocated/1024.0));
    printf("Deallocated: %f GB\n", (size_deallocated/1024.0));
}
