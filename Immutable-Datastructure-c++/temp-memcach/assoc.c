...
void assoc_init(void) {
    ...
    primary_hashtable = (item**) nvm_get_id("primary_hashtable");
    if(!primary_hashtable) {

        primary_hashtable = (item**) nvm_reserve_id("primary_hashtable", hash_size);
        if(primary_hashtable) {
...
void assoc_expand(void) {
    ...
    primary_hashtable = (item**) nvm_reserve_id("primary_hashtable", hashsize(hashpower + 1) * sizeof(void*));
    if (primary_hashtable) {
...
void do_assoc_move_next_bucket(void) {
    ...
    if (expanding) {
        ...
        if (expand_bucket == hashsize(hashpower - 1)) {
            expanding = false;
            nvm_free(old_hashtable, NULL, NULL, NULL, NULL);
