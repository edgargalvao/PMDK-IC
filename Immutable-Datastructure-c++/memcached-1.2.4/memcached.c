...
static void complete_nread(conn *c) {
    ...
    item_remove(c->item_ptr);       /* release the c->item reference */
    c->item_ptr = 0;
}
...
static void process_stat(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    if (strcmp(subcommand, "reset") == 0) {
        assert(0 && "stats reset not available");
        // stats_reset();
        out_string(c, "RESET");
        return;
    }
    ...
    if (strcmp(subcommand, "slabs") == 0) {
        int bytes = 0;
        assert(0 && "slabs stats not supported");
    ...
    if (strcmp(subcommand, "detail") == 0) {
        assert(0 && "detailed stats not available");
        /*
        if (ntokens < 4)
...
int main (int argc, char **argv) {
    ...
    if (!BackingFileExists(path)) {
        fprintf(stdout, "Initializing backing file\n");
        nvm_initialize(path, 0);
    } else {
        fprintf(stdout, "Recovering from backing file\n");
